...
export const GaleerySubCategory = () => {
  ...
  const handleEdit = (row) => {
    ...
    window.scrollTo(0, 0);
    //  console.log(newDistrict, "newDistrictttttttttttt");
  ...
  const UpdateData = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${config.url}/admin/updateGallerySubCategory`,
        {
          id: updatedData,
          name: newDistrict,
        },
  ...
  const handleDelete = async (row) => {
    ...
    try {
      const params = {
        ...
        table_name: "gallery_sub_category",
      };
  ...
  const getDistrictEntries = () => {
    ...
    fetch(`${config.url}/admin/getGallerySubCategories`, {
      method: "GET",
      headers: {
  ...
  const handleFormSubmit = async () => {
    try {
      ...
      const res = await fetch(`${config.url}/admin/CreateGallerySubCategory`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${getAllLocatData()?.token}`,
      ...
      if (resJson.status === 200) {
        // Update category in state after successful creation
            ...
            <div id="field0">
              <div className="row align-items-center justify-content-start">
                ...
                <div className="col-md-12 ">
                  <h3 className="mb-4"> गैलरी उप श्रेणी</h3>
                </div>
                <div className="col-md-3 mb-3">
                  <input
                    id="district"
                    name="district"
                    placeholder="उप श्रेणी का नाम"
                    type="text"
                    value={newDistrict}
          ...
          <div className="col">
            <Card className="">
              {/* <CardHeader className="border-0">
                <h3 className="mb-0"> All Sub Categories </h3>
              </CardHeader> */}
                        ...
                        {currentItems?.map((row, i) => (
                          <TableRow key={i}>
                            ...
                            <TableCell>
                              <Switch
                                checked={
                                  row.id === selectedRowStatus.id
                                    ? selectedRowStatus.is_deleted
                                    : row.is_deleted
                                }
                                onChange={() => {
